...
// AdminWalletBalanceService is an optional capability so existing upstream
// admin test doubles retain their narrow AdminService contract. The production
// implementation is always wired with a RedstoneWalletPort.
type AdminWalletBalanceService interface {
	UpdateUserWalletBalance(ctx context.Context, userID int64, balance float64, operation string, asset RedstoneWalletAsset, notes, idempotencyKey string) (*User, error)
}
...
func (s *adminServiceImpl) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
