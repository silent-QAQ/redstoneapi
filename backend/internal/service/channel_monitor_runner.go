...
// SetClusterLeaseCoordinator installs optional cross-node ownership after the
// runner has been constructed. It is called during dependency wiring, before
// Start, so a disabled cluster retains the original runner behavior.
func (r *ChannelMonitorRunner) SetClusterLeaseCoordinator(coordinator ClusterTaskLeaseCoordinator) {
	if r == nil {
		return
	}
	r.mu.Lock()
	r.leaseCoordinator = coordinator
	r.mu.Unlock()
}
...
func (r *ChannelMonitorRunner) runOne(id int64, name string) {
	...
	r.mu.Lock()
	coordinator := r.leaseCoordinator
	r.mu.Unlock()

	runCheck := func(checkCtx context.Context) error {
		_, err := r.svc.RunCheck(checkCtx, id)
		return err
	}

	if coordinator != nil && coordinator.Enabled() {
		acquired, err := coordinator.RunWithTaskLease(ctx, fmt.Sprintf("channel-monitor:active:%d", id), runCheck)
		if !acquired {
			if err != nil {
				slog.Warn("channel_monitor: acquire cluster lease failed", "monitor_id", id, "name", name, "error", err)
			}
			return
		}
		if err == nil {
		...
		slog.Warn("channel_monitor: cluster-leased check failed", "monitor_id", id, "name", name, "error", err)
		return
	}
