...
// SetRedstoneWalletPort is wired only in production. Keeping the constructor
// stable avoids breaking upstream service tests while ensuring the configured
// runtime never uses direct balance writes for wallet-aware redemption.
func (s *RedeemService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
...
func (s *RedeemService) GenerateCodes(ctx context.Context, req GenerateCodesRequest) ([]RedeemCode, error) {
	...
	if codeType == RedeemTypeBoundBalance && value < 0 {
		return nil, errors.New("bound balance redeem code value must be positive")
	}
...
func (s *RedeemService) CreateCode(ctx context.Context, code *RedeemCode) error {
	...
	if code.Type == RedeemTypeBoundBalance && code.Value < 0 {
		return errors.New("bound balance redeem code value must be positive")
	}
	if code.Status == "" {
...
func (s *RedeemService) Redeem(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	...
	case RedeemTypeBalance, RedeemTypeBoundBalance, RedeemTypeConcurrency:
	...
	case RedeemTypeBalance:
		...
		if s.walletPort == nil {
			return nil, errors.New("redstone wallet is not configured for balance redemption")
		}
		command := RedstoneWalletCreditCommand{
				UserID:         userID,
				Asset:          RedstoneWalletAssetNormal,
				Amount:         amount,
				Reason:         "redeem_code",
				ReferenceType:  "redeem_code",
				ReferenceID:    strconv.FormatInt(redeemCode.ID, 10),
				IdempotencyKey: "redeem-code-" + strconv.FormatInt(redeemCode.ID, 10),
			}
		if amount < 0 {
			if err := s.walletPort.AdjustNormalInExecutor(txCtx, tx.Client(), RedstoneWalletAdjustmentCommand{
					UserID:         userID,
					Delta:          amount,
		...
		} else if err := s.walletPort.CreditInExecutor(txCtx, tx.Client(), command); err != nil {
				return nil, fmt.Errorf("credit wallet balance: %w", err)
		}

	case RedeemTypeBoundBalance:
		if s.walletPort == nil {
			return nil, errors.New("wallet is not configured for bound balance redemption")
		}
		if redeemCode.Value <= 0 {
			return nil, infraerrors.BadRequest("REDEEM_CODE_INVALID", "bound balance redeem code value must be positive")
		}
		if err := s.walletPort.CreditInExecutor(txCtx, tx.Client(), RedstoneWalletCreditCommand{
			UserID:         userID,
			...
			return nil, fmt.Errorf("credit bound wallet balance: %w", err)
		}
...
func (s *RedeemService) invalidateRedeemCaches(ctx context.Context, userID int64, redeemCode *RedeemCode) {
	switch redeemCode.Type {
	case RedeemTypeBalance, RedeemTypeBoundBalance:
