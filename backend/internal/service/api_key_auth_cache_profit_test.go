...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, 21, snapshot.Version, "v21 起认证快照携带 Redstone 房间模式倍率")
	require.NotNil(t, snapshot.SharingRoomID)
	require.Equal(t, int64(70), *snapshot.SharingRoomID)
	...
	require.NotNil(t, materialized.SharingRoomID)
	require.Equal(t, int64(70), *materialized.SharingRoomID)
