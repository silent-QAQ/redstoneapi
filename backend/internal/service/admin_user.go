...
import (
	...
	"github.com/silent-QAQ/redstoneapi/ent/authidentitychannel"
...
func (s *adminServiceImpl) UpdateUserBalance(ctx context.Context, userID int64, balance float64, operation string, notes string) (*User, error) {
	if s == nil || s.walletPort == nil {
		return nil, errors.New("redstone wallet is not configured")
	}
	if userID <= 0 || balance < 0 {
		return nil, errors.New("user and non-negative amount are required")
	}
	keySeed := fmt.Sprintf("legacy-admin-balance|%d|%s|%.8f|%s", userID, operation, balance, notes)
	return s.UpdateUserWalletBalance(ctx, userID, balance, operation, RedstoneWalletAssetNormal, notes, keySeed)
}

// UpdateUserWalletBalance is the Redstone replacement for direct admin
// balance mutations. It uses durable wallet operation keys rather than the
// Redis coordinator TTL, and every non-zero change has an immutable ledger
// entry. Bound balance accepts only administrator grants.
func (s *adminServiceImpl) UpdateUserWalletBalance(
	ctx context.Context,
	userID int64,
	balance float64,
	operation string,
	...
	if userID <= 0 || balance < 0 || strings.TrimSpace(idempotencyKey) == "" || (operation != "set" && balance == 0) {
		return nil, errors.New("user, amount and idempotency key are required")
	}
	if asset == "" {
		asset = RedstoneWalletAssetNormal
	}
	key := adminWalletOperationKey(userID, idempotencyKey)
	referenceID := key
	if asset == RedstoneWalletAssetBound {
		if operation != "add" {
			return nil, errors.New("bound balance only supports add")
		}
		if err := s.walletPort.Credit(ctx, RedstoneWalletCreditCommand{
			UserID:         userID,
			Asset:          RedstoneWalletAssetBound,
			Amount:         balance,
			Reason:         "admin_grant",
	...
	} else if asset != RedstoneWalletAssetNormal {
		return nil, errors.New("unsupported wallet asset")
	} else {
		var err error
		switch operation {
		case "add":
			err = s.walletPort.Credit(ctx, RedstoneWalletCreditCommand{
				UserID: userID, Asset: RedstoneWalletAssetNormal, Amount: balance, Reason: "admin_grant",
				ReferenceType: "admin_balance_adjustment", ReferenceID: referenceID, IdempotencyKey: key,
			})
		case "subtract":
			err = s.walletPort.AdjustNormal(ctx, RedstoneWalletAdjustmentCommand{
				UserID: userID, Delta: -balance, Operation: "admin_adjustment",
				ReferenceType: "admin_balance_adjustment", ReferenceID: referenceID, IdempotencyKey: key,
			})
		case "set":
			err = s.walletPort.SetNormal(ctx, RedstoneWalletSetNormalCommand{
	...
	if asset == RedstoneWalletAssetNormal && operation == "add" {
		s.tryAccrueAffiliateRebateForAdminRecharge(ctx, userID, operation, balance)
	}
	_ = notes // ledger references are stable identifiers; free-form notes stay in existing admin audit middleware.
	return user, nil
}

func adminWalletOperationKey(userID int64, idempotencyKey string) string {
	sum := sha256.Sum256([]byte(strconv.FormatInt(userID, 10) + "\x00" + strings.TrimSpace(idempotencyKey)))
	return "admin-wallet-" + fmt.Sprintf("%x", sum[:])
}
