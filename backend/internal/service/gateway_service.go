...
type GatewayService struct {
	...
	modelsListCacheEpochMu     sync.Mutex
	modelsListCacheEpochReader GatewayCacheEpochReader
	modelsListCacheEpoch       int64
	modelsListCacheEpochSet    bool
...
// SetSharingAccessGuard installs Redstone's account-level sharing lease guard.
// It is optional so existing service constructors and non-gateway jobs remain
// source-compatible; production wiring always provides it.
func (s *GatewayService) SetSharingAccessGuard(guard SharingAccessGuard) {
	if s != nil {
		s.sharingAccessGuard = guard
	}
}

func (s *GatewayService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}

const gatewayAvailableModelsCacheEpochName = "gateway:available-models"

// SetAvailableModelsCacheEpochReader makes the in-process model-list cache
// observe durable Redstone cache epochs. It is installed by the HTTP runtime,
// leaving direct service construction and single-node deployments unchanged.
func (s *GatewayService) SetAvailableModelsCacheEpochReader(reader GatewayCacheEpochReader) {
	if s == nil {
		return
	}
	s.modelsListCacheEpochMu.Lock()
	s.modelsListCacheEpochReader = reader
	s.modelsListCacheEpochSet = false
	s.modelsListCacheEpoch = 0
	s.modelsListCacheEpochMu.Unlock()
}

func (s *GatewayService) syncAvailableModelsCacheEpoch(ctx context.Context) {
	if s == nil || s.modelsListCache == nil {
		return
	}
	s.modelsListCacheEpochMu.Lock()
	reader := s.modelsListCacheEpochReader
	s.modelsListCacheEpochMu.Unlock()
	if reader == nil || !reader.Enabled() {
		return
	}

	epoch, err := reader.CacheEpoch(ctx, gatewayAvailableModelsCacheEpochName)
	if err != nil || epoch <= 0 {
		if err != nil {
			slog.Debug("gateway available-models epoch read failed", "error", err)
		}
		return
	}

	s.modelsListCacheEpochMu.Lock()
	defer s.modelsListCacheEpochMu.Unlock()
	if s.modelsListCacheEpochSet && s.modelsListCacheEpoch == epoch {
		return
	}
	s.modelsListCache.Flush()
	s.modelsListCacheEpoch = epoch
	s.modelsListCacheEpochSet = true
}
...
func (s *GatewayService) DoGrokNativeResponsesJSON(ctx context.Context, account *Account, body []byte) ([]byte, error) {
	if s == nil {
		return nil, errors.New("gateway service not configured")
	}
	if account == nil {
		return nil, errors.New("account is required")
	}
	if err := ensureAccountSharingAccess(ctx, s.sharingAccessGuard, account); err != nil {
		return nil, err
	}
...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	s.syncAvailableModelsCacheEpoch(ctx)
