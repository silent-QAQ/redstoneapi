...
func (s *BillingCacheService) checkBalanceEligibility(ctx context.Context, userID int64) error {
	balance, err := s.getTokenChargeEligibilityBalance(ctx, userID)
	if err != nil {
...
// getTokenChargeEligibilityBalance reads bound balance only for token billing
// eligibility. The regular BillingCache remains normal-balance-only so its
// payment, withdrawal, and low-balance notification consumers keep their
// established semantics.
func (s *BillingCacheService) getTokenChargeEligibilityBalance(ctx context.Context, userID int64) (float64, error) {
	if loader, ok := s.userRepo.(TokenBalanceEligibilityLoader); ok {
		balances, err := loader.GetTokenBalanceEligibility(ctx, userID)
		if err != nil {
			return 0, err
		}
		return balances.AvailableForTokenCharge(), nil
	}

	// Preserve the legacy path for test doubles and deployments that have not
	// applied the Redstone wallet migration yet.
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID int64, group *Group, subscription *UserSubscription) error {
	...
	if group.HasDailyLimit() && subData.DailyUsage >= *group.DailyLimitUSD {
		return s.subscriptionWalletFallbackEligibility(ctx, userID, ErrDailyLimitExceeded)
	}

	if group.HasWeeklyLimit() && subData.WeeklyUsage >= *group.WeeklyLimitUSD {
		return s.subscriptionWalletFallbackEligibility(ctx, userID, ErrWeeklyLimitExceeded)
	}

	if group.HasMonthlyLimit() && subData.MonthlyUsage >= *group.MonthlyLimitUSD {
		return s.subscriptionWalletFallbackEligibility(ctx, userID, ErrMonthlyLimitExceeded)
	}

	return nil
}

// subscriptionWalletFallbackEligibility keeps an exhausted subscription
// usable when a token wallet can cover the request. The definitive amount and
// the subscription-to-wallet split are still decided under DB row locks by
// UsageBillingRepository.Apply; this is only an admission pre-check.
func (s *BillingCacheService) subscriptionWalletFallbackEligibility(ctx context.Context, userID int64, exhausted error) error {
	balance, err := s.getTokenChargeEligibilityBalance(ctx, userID)
	if err != nil {
		return ErrBillingServiceUnavailable.WithCause(err)
	}
