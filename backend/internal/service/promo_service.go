...
func (s *PromoService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
...
func (s *PromoService) ApplyPromoCode(ctx context.Context, userID int64, code string) error {
	...
	// The promo usage, its use counter, and the immutable wallet receipt share
	// one transaction. A configured runtime must never fall back to a direct
	// users.balance mutation.
	if promoCode.BonusAmount != 0 {
		if s.walletPort == nil {
			return fmt.Errorf("credit promo wallet: redstone wallet is not configured")
		}
		walletKey := fmt.Sprintf("promo-code-%d-%d", promoCode.ID, userID)
		if promoCode.BonusAmount > 0 {
			err = s.walletPort.CreditInExecutor(txCtx, tx.Client(), RedstoneWalletCreditCommand{
				UserID: userID, Asset: RedstoneWalletAssetNormal, Amount: promoCode.BonusAmount,
				Reason: "promo_code", ReferenceType: "promo_code", ReferenceID: fmt.Sprint(promoCode.ID),
				IdempotencyKey: walletKey,
			})
		} else {
			err = s.walletPort.AdjustNormalInExecutor(txCtx, tx.Client(), RedstoneWalletAdjustmentCommand{
				UserID: userID, Delta: promoCode.BonusAmount, Operation: "promo_code",
				ReferenceType: "promo_code", ReferenceID: fmt.Sprint(promoCode.ID), IdempotencyKey: walletKey,
			})
		}
		if err != nil {
			return fmt.Errorf("credit promo wallet: %w", err)
		}
	}
