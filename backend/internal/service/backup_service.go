...
// SetClusterLeaseCoordinator ensures the scheduled backup is executed by one
// healthy Redstone node. Manually started backups keep their established local
// lock behavior and are intentionally not made unavailable during a drain.
func (s *BackupService) SetClusterLeaseCoordinator(coordinator ClusterTaskLeaseCoordinator) {
	if s == nil {
		return
	}
	s.cronMu.Lock()
...
func (s *BackupService) runScheduledBackup() {
	...
	if coordinator != nil && coordinator.Enabled() {
		acquired, err := coordinator.RunWithTaskLease(ctx, "scheduled-backup", func(leaseCtx context.Context) error {
			s.runScheduledBackupOnce(leaseCtx)
			return nil
		})
		if err != nil {
			logger.LegacyPrintf("service.backup", "[Backup] cluster lease failed: %v", err)
		}
		if !acquired || err != nil {
			return
		}
	...
	s.runScheduledBackupOnce(ctx)
}

func (s *BackupService) runScheduledBackupOnce(ctx context.Context) {
	// 读取定时备份配置中的过期天数
