...
import (
	...
	"github.com/silent-QAQ/redstoneapi/ent/paymentauditlog"
	"github.com/silent-QAQ/redstoneapi/ent/paymentorder"
	"github.com/silent-QAQ/redstoneapi/ent/paymentproviderinstance"
	"github.com/silent-QAQ/redstoneapi/internal/payment"
	"github.com/silent-QAQ/redstoneapi/internal/payment/provider"
	infraerrors "github.com/silent-QAQ/redstoneapi/internal/pkg/errors"
	"github.com/silent-QAQ/redstoneapi/internal/pkg/servertiming"
)
...
func (s *PaymentService) deductRefundWalletBalance(ctx context.Context, p *RefundPlan, executor RedstoneWalletExecutor) (float64, error) {
	if p == nil || p.Order == nil || p.BalanceToDeduct <= 0 {
		return 0, nil
	}
	if s.walletPort == nil {
		return 0, errors.New("redstone wallet is not configured for refund deduction")
	}
	if p.WalletDeductionKey == "" {
		p.WalletDeductionKey = fmt.Sprintf("refund-debit-%d-%d", p.OrderID, time.Now().UTC().UnixNano())
	}
	command := RedstoneWalletAdjustmentCommand{
		UserID:         p.Order.UserID,
		Delta:          -p.BalanceToDeduct,
		Operation:      "refund",
		ReferenceType:  "payment_order",
		ReferenceID:    strconv.FormatInt(p.OrderID, 10),
		IdempotencyKey: p.WalletDeductionKey,
	}
	var (
		deducted float64
		err      error
	...
	if executor != nil {
		deducted, err = s.walletPort.DeductNormalAvailableInExecutor(ctx, executor, command)
	} else {
		deducted, err = s.walletPort.DeductNormalAvailable(ctx, command)
	}
	if err != nil {
		return 0, err
	}
...
func (s *PaymentService) ExecuteRefund(ctx context.Context, p *RefundPlan) (*RefundResult, error) {
	...
	if p.DeductionType == payment.DeductionTypeBalance && p.BalanceToDeduct > 0 {
		...
		if !s.hasAuditLog(ctx, p.OrderID, "REFUND_ROLLBACK_FAILED") {
			deducted, err := s.deductRefundWalletBalance(ctx, p, nil)
			if err != nil {
...
func (s *PaymentService) finalizePendingRefundSuccess(ctx context.Context, p *RefundPlan) (_ *RefundResult, err error) {
	...
	if err := s.applyRefundFinalDeduction(txCtx, p, tx.Client()); err != nil {
		return nil, err
	}
...
func (s *PaymentService) applyRefundFinalDeduction(ctx context.Context, p *RefundPlan, executor RedstoneWalletExecutor) error {
	if p.DeductionType == payment.DeductionTypeBalance && p.BalanceToDeduct > 0 {
		if p.WalletDeductionKey == "" && s.walletPort != nil {
			p.WalletDeductionKey = "refund-final-debit-" + strconv.FormatInt(p.OrderID, 10)
		}
		deducted, err := s.deductRefundWalletBalance(ctx, p, executor)
		if err != nil {
...
func (s *PaymentService) RollbackRefund(ctx context.Context, p *RefundPlan, gErr error) bool {
	if p.DeductionType == payment.DeductionTypeBalance && p.BalanceToDeduct > 0 {
		var err error
		if s.walletPort != nil {
			rollbackKey := p.WalletDeductionKey + "-rollback"
			if p.WalletDeductionKey == "" {
				rollbackKey = fmt.Sprintf("refund-rollback-%d-%d", p.OrderID, time.Now().UTC().UnixNano())
			}
			...
			err = errors.New("redstone wallet is not configured for refund rollback")
		}
		if err != nil {
