...
// postUsageBilling is a compatibility path for tests and temporary degraded
// deployments. It retains the non-wallet side effects but routes every token
// debit through the Redstone wallet; there is deliberately no direct balance
// deduction when the wallet or unified billing repository is unavailable.
func postUsageBilling(ctx context.Context, requestID string, p *postUsageBillingParams, deps *billingDeps) error {
	...
	if p.IsSubscriptionBill {
		...
		if cost.ActualCost > 0 {
			if deps.walletPort == nil {
				return fmt.Errorf("gateway wallet is not configured")
			}
			if p.User == nil || p.APIKey == nil {
				return fmt.Errorf("gateway wallet charge requires user and api key")
			}
			charge, err := deps.walletPort.ChargeToken(billingCtx, RedstoneWalletTokenChargeCommand{
				UserID: p.User.ID, Amount: cost.ActualCost,
				ReferenceType: "usage_billing", ReferenceID: gatewayFallbackWalletKey(requestID, p.User.ID, p.APIKey.ID),
				IdempotencyKey: gatewayFallbackWalletKey(requestID, p.User.ID, p.APIKey.ID),
			})
			if err != nil {
				return fmt.Errorf("charge gateway wallet: %w", err)
			}
			if charge.NormalDebited > 0 && deps.billingCacheService != nil {
				deps.billingCacheService.QueueDeductBalance(p.User.ID, charge.NormalDebited)
			}
		}
	}
...
func gatewayFallbackWalletKey(requestID string, userID, apiKeyID int64) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%d|%d|%s", userID, apiKeyID, strings.TrimSpace(requestID))))
	return "gateway-fallback-" + hex.EncodeToString(sum[:])
}
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	if p.IsSubscriptionBill && p.Subscription != nil && p.Cost.TotalCost > 0 {
		...
		// The repository consumes subscription quota first while holding its row
		// lock, then charges only the uncovered tail through the token wallet.
		// Supplying the same cost here is intentional: it is not a double charge.
...
func applyUsageBilling(ctx context.Context, requestID string, usageLog *UsageLog, p *postUsageBillingParams, deps *billingDeps, repo UsageBillingRepository) (bool, error) {
	...
	if cmd == nil || cmd.RequestID == "" || repo == nil {
		return true, postUsageBilling(ctx, requestID, p, deps)
	}
...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p.IsSubscriptionBill && result != nil && result.SubscriptionCost != nil && *result.SubscriptionCost > 0 && p.User != nil && p.APIKey != nil && p.APIKey.GroupID != nil {
		deps.billingCacheService.QueueUpdateSubscriptionUsage(p.User.ID, *p.APIKey.GroupID, *result.SubscriptionCost)
	}
	if result != nil && result.NormalBalanceCost != nil && *result.NormalBalanceCost > 0 && p.User != nil {
...
func syncBalanceCacheAfterDeduction(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	deducted := p.Cost.ActualCost
	if result != nil && result.NormalBalanceCost != nil {
		deducted = *result.NormalBalanceCost
	}
	if deducted > 0 {
		deps.billingCacheService.QueueDeductBalance(p.User.ID, deducted)
	}
}
...
func notifyBalanceLow(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if result != nil && result.NormalBalanceCost != nil && *result.NormalBalanceCost <= 0 {
		return
	}
...
func resolveOldBalance(p *postUsageBillingParams, result *UsageBillingApplyResult) float64 {
	if result != nil && result.NewBalance != nil {
		deducted := p.Cost.ActualCost
		if result.NormalBalanceCost != nil {
			deducted = *result.NormalBalanceCost
		}
		return *result.NewBalance + deducted
	}
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// Room mode has an explicit pricing snapshot. It defaults to 1x and wins
	// over the mapped private group's ordinary multiplier.
	if apiKey.SharingRoomID != nil && apiKey.SharingRoomRateMultiplier != nil {
		multiplier = *apiKey.SharingRoomRateMultiplier
	}
