...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/platform/liveattestation"
...
func (s *OpenAIGatewayService) SetSharingAccessGuard(guard SharingAccessGuard) {
	if s != nil {
		s.sharingAccessGuard = guard
	}
}

func (s *OpenAIGatewayService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
