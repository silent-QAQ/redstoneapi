...
func (s *PaymentService) doBalance(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) error {
	if s.walletPort != nil {
		// The wallet receipt is a durable saga step keyed by order ID. A crash
		// after this credit but before markCompleted is safe: retrying replays
		// the receipt and only advances the order state.
		...
		if existing != nil && lookupErr == nil {
			// Orders created by a pre-wallet deployment retain their redeem-code
			// state machine so a used historical code is never credited twice.
			if existing.IsUsed() {
			...
			if _, err := s.redeemService.Redeem(ContextSkipRedeemAffiliate(ctx), o.UserID, o.RechargeCode); err != nil {
				return fmt.Errorf("redeem legacy balance: %w", err)
			}
		} else {
			if err := s.walletPort.Credit(ctx, RedstoneWalletCreditCommand{
				...
				ReferenceID:    strconv.FormatInt(o.ID, 10),
				IdempotencyKey: "payment-fulfillment-" + strconv.FormatInt(o.ID, 10),
			}); err != nil {
				return fmt.Errorf("credit payment wallet: %w", err)
			}
		}
		if err := s.applyAffiliateRebateForOrder(ctx, o); err != nil {
			return err
