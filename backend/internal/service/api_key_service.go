...
// apiKeyRoomBindingRepository is intentionally an optional extension of
// APIKeyRepository. Keeping the long-lived core repository interface stable
// avoids forcing every focused test double and third-party adapter to know
// about Redstone while the production repository still implements the full
// binding lifecycle.
type apiKeyRoomBindingRepository interface {
	BindSharingRoom(ctx context.Context, apiKeyID, userID, roomID int64) (*APIKeyRoomBinding, error)
	UnbindSharingRoom(ctx context.Context, apiKeyID, userID int64) error
}
...
// BindSharingRoom pins an API key to an accessible active sharing room. The
// repository changes the room binding and api_keys.group_id in one transaction.
func (s *APIKeyService) BindSharingRoom(ctx context.Context, id, userID, roomID int64) (*APIKey, error) {
	if id <= 0 || userID <= 0 || roomID <= 0 {
		return nil, ErrAPIKeyRoomUnavailable
	}
	repo, ok := s.apiKeyRepo.(apiKeyRoomBindingRepository)
	if !ok {
		return nil, ErrAPIKeyRoomBindingUnavailable
	}
	...
	if _, err := repo.BindSharingRoom(ctx, id, userID, roomID); err != nil {
		return nil, err
	}

	// The binding changes both group and room authorization data. Clear all
	// cache layers before returning so the next gateway request cannot use a
	// stale, unbound snapshot.
	s.InvalidateAuthCacheByKey(ctx, apiKey.Key)
	updated, err := s.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("reload api key after room binding: %w", err)
	}
	return updated, nil
}

// UnbindSharingRoom removes the exact room constraint and clears group_id in
// the same transaction. A manually selected group must be set through the
// normal key update flow after unbinding, which keeps the room/group invariant
// unambiguous.
func (s *APIKeyService) UnbindSharingRoom(ctx context.Context, id, userID int64) (*APIKey, error) {
	if id <= 0 || userID <= 0 {
		return nil, ErrAPIKeyRoomUnavailable
	}
	repo, ok := s.apiKeyRepo.(apiKeyRoomBindingRepository)
	...
	if err := repo.UnbindSharingRoom(ctx, id, userID); err != nil {
		return nil, err
	}
	...
	updated, err := s.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("reload api key after room unbinding: %w", err)
	}
	return updated, nil
}
