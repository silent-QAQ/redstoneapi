...
// SetClusterLeaseCoordinator installs optional durable coordination for the
// passive aggregation worker. The legacy advisory lock remains in place as a
// second line of defense and for deployments with clustering disabled.
func (s *ChannelMonitorV2Aggregator) SetClusterLeaseCoordinator(coordinator ClusterTaskLeaseCoordinator) {
	if s == nil {
		return
	}
	s.mu.Lock()
	s.leaseCoordinator = coordinator
	s.mu.Unlock()
}
...
func (s *ChannelMonitorV2Aggregator) runOnce() {
	...
	defer cancel()
	s.mu.Lock()
	coordinator := s.leaseCoordinator
	s.mu.Unlock()
	if coordinator != nil && coordinator.Enabled() {
		acquired, err := coordinator.RunWithTaskLease(ctx, channelMonitorV2AggregatorLockKey, func(leaseCtx context.Context) error {
			s.runOnceWithLeaderLock(leaseCtx)
			return nil
		})
		if err != nil {
			logger.LegacyPrintf("service.channel_monitor_v2", "[ChannelMonitorV2] cluster lease failed: %v", err)
		}
		if !acquired || err != nil {
			return
		}
	...
	s.runOnceWithLeaderLock(ctx)
}

func (s *ChannelMonitorV2Aggregator) runOnceWithLeaderLock(ctx context.Context) {
