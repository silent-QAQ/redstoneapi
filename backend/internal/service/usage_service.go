...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/pkg/usagestats"
)
...
func (s *UsageService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
...
func (s *UsageService) Create(ctx context.Context, req CreateUsageLogRequest) (*UsageLog, error) {
	...
	if inserted && req.ActualCost > 0 {
		if s.walletPort == nil {
			return nil, fmt.Errorf("charge usage wallet: redstone wallet is not configured")
		}
		walletTx := dbent.TxFromContext(txCtx)
		if walletTx == nil {
			return nil, fmt.Errorf("charge usage wallet: transaction is not available")
		}
		charge, err := s.walletPort.ChargeTokenInExecutor(txCtx, walletTx.Client(), RedstoneWalletTokenChargeCommand{
			UserID: req.UserID, Amount: req.ActualCost,
			ReferenceType: "usage_log", ReferenceID: usageWalletKey(req),
			IdempotencyKey: usageWalletKey(req),
		})
		if err != nil {
			return nil, fmt.Errorf("charge usage wallet: %w", err)
		}
		balanceUpdated = charge.NormalDebited > 0
	}
...
func usageWalletKey(req CreateUsageLogRequest) string {
	value := fmt.Sprintf("%d|%d|%s", req.UserID, req.APIKeyID, req.RequestID)
	sum := sha256.Sum256([]byte(value))
	return "legacy-usage-" + hex.EncodeToString(sum[:])
}
