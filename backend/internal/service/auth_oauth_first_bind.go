...
ON CONFLICT (user_id, provider_type, grant_reason) DO NOTHING`,
	...
	if providerDefaults.Balance != 0 {
		if s.walletPort == nil {
			return fmt.Errorf("apply first bind balance default: redstone wallet is not configured")
		}
		providerType := strings.TrimSpace(providerType)
		commandKey := "provider-first-bind-" + fmt.Sprint(userID) + "-" + providerType
		if providerDefaults.Balance > 0 {
			err = s.walletPort.CreditInExecutor(ctx, client, RedstoneWalletCreditCommand{
				UserID: userID, Asset: RedstoneWalletAssetNormal, Amount: providerDefaults.Balance,
				Reason: "provider_grant", ReferenceType: "provider_first_bind", ReferenceID: commandKey,
				IdempotencyKey: commandKey,
			})
		} else {
			err = s.walletPort.AdjustNormalInExecutor(ctx, client, RedstoneWalletAdjustmentCommand{
				UserID: userID, Delta: providerDefaults.Balance, Operation: "provider_grant",
				ReferenceType: "provider_first_bind", ReferenceID: commandKey, IdempotencyKey: commandKey,
			})
		}
		if err != nil {
			return fmt.Errorf("apply first bind wallet grant: %w", err)
		}
	}
