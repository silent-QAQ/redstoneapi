...
type RefundPlan struct {
	...
	// WalletDeductionKey identifies one refundable wallet debit attempt. A
	// gateway failure is compensated by a new ledger credit, so a retry must
	// receive a fresh debit key rather than replay the compensated command.
	WalletDeductionKey string
}
...
// SetRedstoneWalletPort attaches the command-only wallet adapter used by
// payment fulfillment and refunds. Keeping it as a setter preserves the
// long-lived constructor used by upstream tests and integrations.
func (s *PaymentService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	s.walletPort = port
}
