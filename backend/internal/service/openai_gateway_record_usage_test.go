...
func newOpenAIRecordUsageServiceForTest(usageRepo UsageLogRepository, userRepo UserRepository, subRepo UserSubscriptionRepository, rateRepo UserGroupRateRepository) *OpenAIGatewayService {
	...
	svc.SetRedstoneWalletPort(&legacyUsageWalletPort{userRepo: userRepo})
	return svc
}

// legacyUsageWalletPort is a test-only command port. It preserves the old
// usage test fixture's observable debit counter without allowing production
// code to fall back to UserRepository balance mutation when a wallet is nil.
type legacyUsageWalletPort struct {
	userRepo UserRepository
}

func (*legacyUsageWalletPort) Credit(context.Context, RedstoneWalletCreditCommand) error { return nil }

func (*legacyUsageWalletPort) CreditInExecutor(context.Context, RedstoneWalletExecutor, RedstoneWalletCreditCommand) error {
	return nil
}

func (*legacyUsageWalletPort) AdjustNormal(context.Context, RedstoneWalletAdjustmentCommand) error {
	return nil
}

func (*legacyUsageWalletPort) AdjustNormalInExecutor(context.Context, RedstoneWalletExecutor, RedstoneWalletAdjustmentCommand) error {
	return nil
}

func (*legacyUsageWalletPort) DeductNormalAvailable(context.Context, RedstoneWalletAdjustmentCommand) (float64, error) {
	return 0, nil
}

func (*legacyUsageWalletPort) DeductNormalAvailableInExecutor(context.Context, RedstoneWalletExecutor, RedstoneWalletAdjustmentCommand) (float64, error) {
	return 0, nil
}

func (*legacyUsageWalletPort) SetNormal(context.Context, RedstoneWalletSetNormalCommand) error {
	return nil
}

func (p *legacyUsageWalletPort) ChargeToken(ctx context.Context, command RedstoneWalletTokenChargeCommand) (RedstoneWalletTokenChargeResult, error) {
	if p == nil || p.userRepo == nil {
		return RedstoneWalletTokenChargeResult{}, errors.New("test wallet user repository is required")
	}
	if err := p.userRepo.DeductBalance(ctx, command.UserID, command.Amount); err != nil {
		return RedstoneWalletTokenChargeResult{}, err
	}
	return RedstoneWalletTokenChargeResult{NormalDebited: command.Amount}, nil
}

func (p *legacyUsageWalletPort) ChargeTokenInExecutor(ctx context.Context, _ RedstoneWalletExecutor, command RedstoneWalletTokenChargeCommand) (RedstoneWalletTokenChargeResult, error) {
	return p.ChargeToken(ctx, command)
}
