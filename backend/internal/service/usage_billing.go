...
type UsageBillingApplyResult struct {
	...
	SubscriptionCost     *float64           // amount absorbed by subscription quota in this transaction
	NewBalance           *float64           // post-deduction normal balance (nil = no wallet deduction)
	BoundBalanceCost     *float64           // wallet amount charged from bound balance before normal balance
	NormalBalanceCost    *float64           // amount charged from normal balance after bound balance is consumed
	BalanceOverdrafted   bool               // retained for response compatibility; wallet charges never overdraft
...
type BatchImageBalanceHoldResult struct {
	...
	// The Redstone wallet owns asynchronous token holds. These fields preserve
	// the asset split for callers and recovery workers; a technical release
	// must return each amount to its original balance bucket.
	BoundBalance        *float64
	BoundBalanceHeld    *float64
	NormalBalanceHeld   *float64
	BoundBalanceRefund  *float64
	NormalBalanceRefund *float64
}
