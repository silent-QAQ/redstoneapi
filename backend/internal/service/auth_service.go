...
// SetRedstoneWalletPort installs the only balance-mutation dependency used by
// OAuth first-bind grants. NewAuthService stays source-compatible for tests.
func (s *AuthService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
