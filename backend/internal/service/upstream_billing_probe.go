...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/pkg/tlsfingerprint"
...
// SetClusterLeaseCoordinator adds durable task ownership to the periodic
// multiplier probe. The existing cadence and leader locks are retained for
// compatibility when the cluster feature is disabled.
func (s *UpstreamBillingProbeService) SetClusterLeaseCoordinator(coordinator ClusterTaskLeaseCoordinator) {
	if s == nil {
		return
	}
	s.mu.Lock()
...
func (s *UpstreamBillingProbeService) RunDue(ctx context.Context) error {
	...
	if coordinator != nil && coordinator.Enabled() {
		acquired, err := coordinator.RunWithTaskLease(ctx, "upstream-billing-probe", func(leaseCtx context.Context) error {
			return s.runDue(leaseCtx)
		})
		if err != nil {
			return fmt.Errorf("run upstream billing probe under cluster lease: %w", err)
		}
		if !acquired {
			return nil
		}
