...
// TokenBalanceEligibility is the read model used only by gateway token
// preflight. Bound balance is intentionally excluded from the generic user
// balance API because it cannot pay for withdrawals, marketplace orders, or
// any non-token operation.
type TokenBalanceEligibility struct {
	NormalBalance float64
	BoundBalance  float64
}

func (b TokenBalanceEligibility) AvailableForTokenCharge() float64 {
	return b.NormalBalance + b.BoundBalance
}

// TokenBalanceEligibilityLoader is an optional narrow port. Production user
// repositories implement it with a database read; existing non-Redstone
// repositories retain the legacy normal-balance preflight behaviour.
type TokenBalanceEligibilityLoader interface {
	GetTokenBalanceEligibility(ctx context.Context, userID int64) (TokenBalanceEligibility, error)
}
...
func (s *UserService) SetRedstoneWalletPort(port RedstoneWalletPort) {
	if s != nil {
		s.walletPort = port
	}
}
...
func (s *UserService) UpdateBalance(ctx context.Context, userID int64, amount float64) error {
	...
	if s == nil || s.walletPort == nil {
		return fmt.Errorf("update balance: redstone wallet is not configured")
	}
	key := legacyUserBalanceWalletKey(userID, amount)
	var err error
	if amount > 0 {
		err = s.walletPort.Credit(ctx, RedstoneWalletCreditCommand{
			UserID: userID, Asset: RedstoneWalletAssetNormal, Amount: amount, Reason: "admin_grant",
			ReferenceType: "legacy_user_service", ReferenceID: key, IdempotencyKey: key,
		})
	} else {
		err = s.walletPort.AdjustNormal(ctx, RedstoneWalletAdjustmentCommand{
			UserID: userID, Delta: amount, Operation: "admin_adjustment",
			ReferenceType: "legacy_user_service", ReferenceID: key, IdempotencyKey: key,
		})
	}
	if err != nil {
...
func legacyUserBalanceWalletKey(userID int64, amount float64) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%d|%.8f", userID, amount)))
	return "legacy-user-balance-" + hex.EncodeToString(sum[:])
}
