...
type APIKey struct {
	...
	Name    string
	GroupID *int64
	// SharingRoomID is set when this key is pinned to a Redstone account-sharing
	// room. The gateway uses it in addition to GroupID to prevent a private
	// group that is reused by multiple rooms from crossing room boundaries.
	SharingRoomID *int64
	// SharingRoomRateMultiplier is the pricing snapshot for room mode. Room
	// mode defaults to 1x and must not inherit a mapped group's multiplier.
	SharingRoomRateMultiplier *float64
	Status                    string
	IPWhitelist               []string
	IPBlacklist               []string
...
// APIKeyRoomBinding is the durable relation between an API key and an
// account-sharing room. GroupID is deliberately persisted alongside RoomID so
// gateway authorization can fail closed if a key is later pointed at a
// different group by an out-of-band operation.
type APIKeyRoomBinding struct {
	APIKeyID       int64
	RoomID         int64
	GroupID        int64
