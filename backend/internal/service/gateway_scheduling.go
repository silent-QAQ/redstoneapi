...
func (s *GatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	if s.schedulerSnapshot != nil {
		...
		if err == nil {
			...
			accounts, err = filterAccountsForSharingAccess(ctx, s.sharingAccessGuard, accounts)
			if err != nil {
	...
	if useMixed {
		...
		filtered, err = filterAccountsForSharingAccess(ctx, s.sharingAccessGuard, filtered)
		if err != nil {
			return nil, useMixed, err
		}
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, platform string) (*Account, error) {
	...
	if len(routingAccountIDs) > 0 {
		...
		for i := range accounts {
			...
			if schedGroup != nil && !schedGroup.AllowsOpenAIAccountLevel(acc.AccountLevel) {
				continue
			}
