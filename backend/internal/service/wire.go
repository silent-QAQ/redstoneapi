...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/pkg/antigravity"
	"github.com/silent-QAQ/redstoneapi/internal/pkg/logger"
	"github.com/silent-QAQ/redstoneapi/internal/pkg/xai"
...
// ProvideGatewayService wires Redstone's lease-aware account guard into the
// existing gateway without changing the long-lived public constructor used by
// focused service tests and integrations.
func ProvideGatewayService(
	...
	sharingAccessGuard SharingAccessGuard,
	...
	svc := NewGatewayService(
		...
		compositeResolver, balanceNotifyService, userPlatformQuotaRepo,
	)
	svc.SetSharingAccessGuard(sharingAccessGuard)
	svc.SetRedstoneWalletPort(walletPort)
	return svc
}

// ProvideOpenAIGatewayService is the OpenAI-compatible counterpart to
// ProvideGatewayService and keeps the same account-level lease invariant.
...
// ProvidePromoService wires bonus redemptions through the immutable wallet
// while preserving NewPromoService for focused upstream-compatible tests.
...
// ProvideUsageService keeps the legacy usage-log API transactional while
// routing its balance side effect through the wallet's token charge command.
...
func ProvideOpsCleanupService(
	...
	settingRepo SettingRepository,
	opsService *OpsService,
	leaseCoordinator ClusterTaskLeaseCoordinator,
) *OpsCleanupService {
	svc := NewOpsCleanupService(opsRepo, db, redisClient, cfg, channelMonitorSvc, settingRepo)
	svc.SetClusterLeaseCoordinator(leaseCoordinator)
	svc.Start()
	if opsService != nil {
...
// ProvideRedeemService attaches the Redstone wallet port after constructing
// the legacy-compatible redeem service. RedeemCode.Use and wallet credit then
// run through the same Ent transaction in Redeem.
...
func ProvideAdminService(
	...
	svc := NewAdminService(
		...
		userGroupRateRepo, userRPMCache, billingCacheService, proxyProber, proxyLatencyCache,
		authCacheInvalidator, entClient, settingService, defaultSubAssigner, userSubRepo,
		privacyClientFactory, runtimeBlocker, affiliateService, compositeRouteRepo, compositeResolver,
	)
	if configured, ok := svc.(*adminServiceImpl); ok {
		configured.SetRedstoneWalletPort(walletPort)
	}
	return svc
}
...
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, walletPort RedstoneWalletPort) *PaymentService {
...
func ProvideChannelMonitorRunner(svc *ChannelMonitorService, settingService *SettingService, leaseCoordinator ClusterTaskLeaseCoordinator) *ChannelMonitorRunner {
	r := NewChannelMonitorRunner(svc, settingService)
	r.SetClusterLeaseCoordinator(leaseCoordinator)
	if svc != nil {
...
func ProvideChannelMonitorV2Aggregator(repo ChannelMonitorV2Repository, db *sql.DB, settingService *SettingService, leaseCoordinator ClusterTaskLeaseCoordinator) *ChannelMonitorV2Aggregator {
	aggregator := NewChannelMonitorV2Aggregator(repo, db, settingService)
	aggregator.SetClusterLeaseCoordinator(leaseCoordinator)
