...
type Config struct {
	...
	MarketplaceStorage      MarketplaceStorageConfig      `mapstructure:"marketplace_storage"`
	MarketplaceScanner      MarketplaceScannerConfig      `mapstructure:"marketplace_scanner"`
	MarketplaceSettlement   MarketplaceSettlementConfig   `mapstructure:"marketplace_settlement"`
	SharingLeaseCleanup     SharingLeaseCleanupConfig     `mapstructure:"account_sharing_lease_cleanup"`
	UserAccountSecrets      UserAccountSecretsConfig      `mapstructure:"user_account_secrets"`
	RedstoneCluster         RedstoneClusterConfig         `mapstructure:"redstone_cluster"`
}

// RedstoneClusterConfig controls the optional multi-node coordination plane.
// PostgreSQL remains authoritative for membership and task ownership; Redis only
// accelerates cache epoch distribution.
type RedstoneClusterConfig struct {
	Enabled                  bool   `mapstructure:"enabled"`
	NodeID                   string `mapstructure:"node_id"`
	AdvertiseURL             string `mapstructure:"advertise_url"`
	HeartbeatIntervalSeconds int    `mapstructure:"heartbeat_interval_seconds"`
	NodeTimeoutSeconds       int    `mapstructure:"node_timeout_seconds"`
	LeaseDurationSeconds     int    `mapstructure:"lease_duration_seconds"`
	CacheEpochTTLSeconds     int    `mapstructure:"cache_epoch_ttl_seconds"`
}

func (c RedstoneClusterConfig) Validate() error {
	if !c.Enabled {
		return nil
	}
	if strings.TrimSpace(c.NodeID) == "" {
		return fmt.Errorf("redstone_cluster.node_id must be set when clustering is enabled")
	}
	if c.HeartbeatIntervalSeconds < 1 || c.HeartbeatIntervalSeconds > 300 {
		return fmt.Errorf("redstone_cluster.heartbeat_interval_seconds must be between 1 and 300")
	}
	if c.NodeTimeoutSeconds < c.HeartbeatIntervalSeconds*3 || c.NodeTimeoutSeconds > 1800 {
		return fmt.Errorf("redstone_cluster.node_timeout_seconds must be at least three heartbeats and no more than 1800")
	}
	if c.LeaseDurationSeconds < c.HeartbeatIntervalSeconds*2 || c.LeaseDurationSeconds > 900 {
		return fmt.Errorf("redstone_cluster.lease_duration_seconds must be at least two heartbeats and no more than 900")
	}
	if c.CacheEpochTTLSeconds < 1 || c.CacheEpochTTLSeconds > 3600 {
		return fmt.Errorf("redstone_cluster.cache_epoch_ttl_seconds must be between 1 and 3600")
	}
	return nil
}
...
// MarketplaceStorageConfig configures private S3-compatible ciphertext storage
// for Redstone marketplace deliveries. It is intentionally separate from image
// storage because marketplace objects must never be returned as public or
// presigned URLs and use an independent envelope-encryption KEK.
type MarketplaceStorageConfig struct {
	Enabled                bool              `mapstructure:"enabled"`
	...
	EncryptionKeyVersion   string            `mapstructure:"encryption_key_version"`
	EncryptionKey          string            `mapstructure:"encryption_key"`
	PreviousEncryptionKeys map[string]string `mapstructure:"previous_encryption_keys"`
	// MaxObjectBytes limits plaintext delivery bytes. The storage adapter adds
	// the fixed envelope-encryption overhead when reading/writing ciphertext.
	MaxObjectBytes int64 `mapstructure:"max_object_bytes"`
}

// MarketplaceScannerConfig enables a local ClamAV daemon for encrypted-market
// uploads. The daemon address is deliberately local/infrastructure-owned; the
// service never sends sensitive delivery content to a third-party moderator.
type MarketplaceScannerConfig struct {
	Enabled               bool   `mapstructure:"enabled"`
...
// MarketplaceSettlementConfig controls the local runtime that releases
// delivered marketplace orders after their dispute window. Financial idempotency
// remains in PostgreSQL, so multiple application nodes can run this safely.
type MarketplaceSettlementConfig struct {
	Enabled         bool `mapstructure:"enabled"`
	IntervalSeconds int  `mapstructure:"interval_seconds"`
	BatchSize       int  `mapstructure:"batch_size"`
}

// SharingLeaseCleanupConfig controls the database-backed worker that
// revokes expired or idle sharing leases. PostgreSQL locks make it safe for
// all application nodes to enable this simultaneously.
type SharingLeaseCleanupConfig struct {
	Enabled         bool `mapstructure:"enabled"`
	IntervalSeconds int  `mapstructure:"interval_seconds"`
	BatchSize       int  `mapstructure:"batch_size"`
}

// UserAccountSecretsConfig provides the dedicated KEK used to envelope-encrypt
// user-controlled OAuth/API credentials. It must never reuse the TOTP,
// payment, or marketplace-delivery key.
type UserAccountSecretsConfig struct {
	Enabled              bool   `mapstructure:"enabled"`
	EncryptionKeyVersion string `mapstructure:"encryption_key_version"`
	EncryptionKey        string `mapstructure:"encryption_key"`
}

func (c *UserAccountSecretsConfig) Active() bool {
	return c != nil && c.Enabled && c.EncryptionKeyVersion != "" && c.EncryptionKey != ""
}

func (c *MarketplaceScannerConfig) Active() bool {
	return c != nil && c.Enabled && c.ClamdAddress != ""
}

func (c *MarketplaceStorageConfig) IsConfigured() bool {
	return c != nil && c.Bucket != "" && c.AccessKeyID != "" && c.SecretAccessKey != "" &&
		c.EncryptionKeyVersion != "" && c.EncryptionKey != ""
}

func (c *MarketplaceStorageConfig) Active() bool {
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if err := viper.ReadInConfig(); err != nil {
		...
		fmt.Printf("[DEBUG Viper] Successfully read config from: %s\n", viper.ConfigFileUsed())
	}
	fmt.Printf("[DEBUG Viper] marketplace_storage.encryption_key=%q\n", viper.GetString("marketplace_storage.encryption_key"))
	...
	fmt.Printf("[DEBUG Config] MarketplaceStorage.Enabled=%v EncryptionKey=%q (len=%d)\n", cfg.MarketplaceStorage.Enabled, cfg.MarketplaceStorage.EncryptionKey, len(cfg.MarketplaceStorage.EncryptionKey))
...
func setDefaults() {
	...
	// Redstone cluster. Disabled by default so single-node deployments retain
	// their existing startup and health-check behavior.
	viper.SetDefault("redstone_cluster.enabled", false)
	viper.SetDefault("redstone_cluster.node_id", "")
	viper.SetDefault("redstone_cluster.advertise_url", "")
	viper.SetDefault("redstone_cluster.heartbeat_interval_seconds", 10)
	viper.SetDefault("redstone_cluster.node_timeout_seconds", 45)
	viper.SetDefault("redstone_cluster.lease_duration_seconds", 30)
	viper.SetDefault("redstone_cluster.cache_epoch_ttl_seconds", 60)
	...
	// Marketplace storage is private ciphertext storage. Keep all fields
	// registered so environment-only deployments can configure it without
	// silently dropping credentials during viper.Unmarshal.
	viper.SetDefault("marketplace_storage.enabled", false)
	viper.SetDefault("marketplace_storage.endpoint", "")
	viper.SetDefault("marketplace_storage.region", "auto")
	viper.SetDefault("marketplace_storage.bucket", "")
	viper.SetDefault("marketplace_storage.access_key_id", "")
	viper.SetDefault("marketplace_storage.secret_access_key", "")
	viper.SetDefault("marketplace_storage.prefix", "redstone-market/")
	viper.SetDefault("marketplace_storage.force_path_style", false)
	viper.SetDefault("marketplace_storage.encryption_key_version", "market-kek-v1")
	viper.SetDefault("marketplace_storage.encryption_key", "")
	viper.SetDefault("marketplace_storage.previous_encryption_keys", map[string]string{})
	viper.SetDefault("marketplace_storage.max_object_bytes", 33554432)
	viper.SetDefault("marketplace_scanner.enabled", false)
	viper.SetDefault("marketplace_scanner.clamd_address", "")
	viper.SetDefault("marketplace_scanner.worker_enabled", true)
	viper.SetDefault("marketplace_scanner.worker_interval_seconds", 15)
	viper.SetDefault("marketplace_scanner.worker_batch_size", 25)
	viper.SetDefault("marketplace_settlement.enabled", true)
	viper.SetDefault("marketplace_settlement.interval_seconds", 60)
	viper.SetDefault("marketplace_settlement.batch_size", 50)
	viper.SetDefault("account_sharing_lease_cleanup.enabled", true)
	viper.SetDefault("account_sharing_lease_cleanup.interval_seconds", 30)
	viper.SetDefault("account_sharing_lease_cleanup.batch_size", 50)
	// User-controlled account secrets use a separate KEK. Register all fields
	// so environment-only deployments cannot silently disable this isolation.
	viper.SetDefault("user_account_secrets.enabled", false)
	viper.SetDefault("user_account_secrets.encryption_key_version", "user-account-kek-v1")
	viper.SetDefault("user_account_secrets.encryption_key", "")
...
func (c *Config) Validate() error {
	...
	if err := c.RedstoneCluster.Validate(); err != nil {
		return err
	}
	if c.MarketplaceSettlement.Enabled {
		if c.MarketplaceSettlement.IntervalSeconds == 0 {
			c.MarketplaceSettlement.IntervalSeconds = 60
		}
		if c.MarketplaceSettlement.BatchSize == 0 {
			c.MarketplaceSettlement.BatchSize = 50
		}
		if c.MarketplaceSettlement.IntervalSeconds < 5 || c.MarketplaceSettlement.IntervalSeconds > 3600 {
			return fmt.Errorf("marketplace_settlement.interval_seconds must be between 5 and 3600")
		}
		if c.MarketplaceSettlement.BatchSize < 1 || c.MarketplaceSettlement.BatchSize > 100 {
			return fmt.Errorf("marketplace_settlement.batch_size must be between 1 and 100")
		}
	}
	if c.SharingLeaseCleanup.Enabled {
		if c.SharingLeaseCleanup.IntervalSeconds == 0 {
			c.SharingLeaseCleanup.IntervalSeconds = 30
		}
		if c.SharingLeaseCleanup.BatchSize == 0 {
			c.SharingLeaseCleanup.BatchSize = 50
		}
		if c.SharingLeaseCleanup.IntervalSeconds < 5 || c.SharingLeaseCleanup.IntervalSeconds > 3600 {
			return fmt.Errorf("account_sharing_lease_cleanup.interval_seconds must be between 5 and 3600")
		}
		if c.SharingLeaseCleanup.BatchSize < 1 || c.SharingLeaseCleanup.BatchSize > 100 {
			return fmt.Errorf("account_sharing_lease_cleanup.batch_size must be between 1 and 100")
		}
	}
	if c.MarketplaceScanner.Enabled {
		if strings.TrimSpace(c.MarketplaceScanner.ClamdAddress) == "" {
			return fmt.Errorf("marketplace_scanner.clamd_address is required when scanner is enabled")
		}
		if c.MarketplaceScanner.WorkerEnabled {
			if c.MarketplaceScanner.WorkerIntervalSeconds < 5 || c.MarketplaceScanner.WorkerIntervalSeconds > 3600 {
				return fmt.Errorf("marketplace_scanner.worker_interval_seconds must be between 5 and 3600")
			}
			if c.MarketplaceScanner.WorkerBatchSize < 1 || c.MarketplaceScanner.WorkerBatchSize > 100 {
				return fmt.Errorf("marketplace_scanner.worker_batch_size must be between 1 and 100")
			}
		}
	}
