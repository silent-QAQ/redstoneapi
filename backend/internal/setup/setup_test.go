...
func TestBuildDatabaseConnectionDSNsUsesPostgresForBootstrap(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("parse bootstrap DSN: %v", err)
	}
	...
	if targetURL.Query().Get("sslmode") != "disable" {
		t.Fatalf("target sslmode = %q, want disable", targetURL.Query().Get("sslmode"))
	}
}

func TestBuildPostgresDSNEncodesEmptyAndSpecialCharacterPasswords(t *testing.T) {
	base := &DatabaseConfig{
		Host: "127.0.0.1", Port: 5432, User: "app user", DBName: "target", SSLMode: "disable",
	}

	emptyPasswordURL, err := url.Parse(buildPostgresDSN(base, "postgres"))
	if err != nil {
		t.Fatalf("parse empty-password DSN: %v", err)
	}
	if emptyPasswordURL.Path != "/postgres" || emptyPasswordURL.User.Username() != "app user" {
		t.Fatalf("empty-password DSN = %q, want postgres database and user", emptyPasswordURL.String())
	}
	if _, present := emptyPasswordURL.User.Password(); present {
		t.Fatalf("empty-password DSN unexpectedly contains a password")
	}

	base.Password = "p ass 'word&="
	passwordURL, err := url.Parse(buildPostgresDSN(base, "target"))
	if err != nil {
		t.Fatalf("parse password DSN: %v", err)
	}
	password, present := passwordURL.User.Password()
	if !present || password != base.Password {
		t.Fatalf("password round-trip = %q, present=%t", password, present)
	}
}
