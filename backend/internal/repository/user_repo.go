...
import (
	...
	dbgroup "github.com/silent-QAQ/redstoneapi/ent/group"
	"github.com/silent-QAQ/redstoneapi/ent/identityadoptiondecision"
	"github.com/silent-QAQ/redstoneapi/ent/predicate"
	"github.com/silent-QAQ/redstoneapi/ent/schema/mixins"
	dbuser "github.com/silent-QAQ/redstoneapi/ent/user"
	"github.com/silent-QAQ/redstoneapi/ent/userallowedgroup"
	"github.com/silent-QAQ/redstoneapi/ent/usersubscription"
...
// GetTokenBalanceEligibility returns the two balances that are spendable by
// gateway token charging. It deliberately bypasses the ordinary balance cache:
// that cache represents only withdrawable normal balance and is also consumed
// by payment and low-balance notification flows.
func (r *userRepository) GetTokenBalanceEligibility(ctx context.Context, userID int64) (service.TokenBalanceEligibility, error) {
	...
	`, userID)
	if err != nil {
		return service.TokenBalanceEligibility{}, err
	}
	defer rows.Close()
	...
	if !rows.Next() {
		...
		return service.TokenBalanceEligibility{}, service.ErrUserNotFound
	}

	var balances service.TokenBalanceEligibility
	if err := rows.Scan(&balances.NormalBalance, &balances.BoundBalance); err != nil {
		return service.TokenBalanceEligibility{}, err
	}
	if err := rows.Err(); err != nil {
		return service.TokenBalanceEligibility{}, err
