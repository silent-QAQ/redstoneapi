...
func (r *apiKeyRepository) GetByID(ctx context.Context, id int64) (*service.APIKey, error) {
	...
	if err := r.attachSharingRoomID(ctx, out); err != nil {
		return nil, err
	}
	return out, nil
}
...
func (r *apiKeyRepository) ListByUserID(ctx context.Context, userID int64, params pagination.PaginationParams, filters service.APIKeyListFilters) ([]service.APIKey, *pagination.PaginationResult, error) {
	...
	if err := r.attachSharingRoomIDs(ctx, outKeys); err != nil {
		return nil, nil, err
	}
...
// attachSharingRoomID keeps the ordinary Ent API-key projection unchanged and
// enriches it from Redstone's independent binding table. This avoids coupling
// sub2's generated APIKey entity to a Redstone-owned lifecycle table.
func (r *apiKeyRepository) attachSharingRoomID(ctx context.Context, key *service.APIKey) error {
	if key == nil {
		return nil
	}
	bindings, err := r.listSharingRoomBindings(ctx, []int64{key.ID})
	if err != nil {
		return err
	}
	if binding, ok := bindings[key.ID]; ok {
		roomID := binding.RoomID
		key.SharingRoomID = &roomID
		rate := binding.RateMultiplier
		key.SharingRoomRateMultiplier = &rate
	}
	return nil
}

func (r *apiKeyRepository) attachSharingRoomIDs(ctx context.Context, keys []service.APIKey) error {
	if len(keys) == 0 {
		return nil
	}
	...
	bindings, err := r.listSharingRoomBindings(ctx, ids)
	if err != nil {
		return err
	}
	for i := range keys {
		if binding, ok := bindings[keys[i].ID]; ok {
			roomID := binding.RoomID
			keys[i].SharingRoomID = &roomID
			rate := binding.RateMultiplier
			keys[i].SharingRoomRateMultiplier = &rate
		}
	}
	return nil
}

func (r *apiKeyRepository) listSharingRoomBindings(ctx context.Context, apiKeyIDs []int64) (map[int64]service.APIKeyRoomBinding, error) {
	// Redstone room mode is PostgreSQL-only. Unit tests and local Ent
	// projections may use SQLite without running Redstone migrations; treat
	// that projection as empty rather than breaking ordinary API-key reads.
	if len(apiKeyIDs) == 0 || r.sql == nil || r.client.Driver().Dialect() != dialect.Postgres {
		return map[int64]service.APIKeyRoomBinding{}, nil
	}
	query, args := apiKeyRoomBindingsQuery(apiKeyIDs, r.client.Driver().Dialect())
	rows, err := r.sql.QueryContext(ctx, query, args...)
	if err != nil {
		// Redstone migrations are intentionally independent from the upstream
		// schema. During a rolling upgrade (and in upstream SQLite tests) the
		// optional binding table may not exist yet; that means no key can be
		// room-bound in this database, so an empty projection is correct. Other
		// errors remain fail-closed and are never masked.
		if isMissingAPIKeyRoomBindingTable(err) {
			return map[int64]service.APIKeyRoomBinding{}, nil
		}
		return nil, err
	}
	...
	bindings := make(map[int64]service.APIKeyRoomBinding, len(apiKeyIDs))
	for rows.Next() {
		binding := service.APIKeyRoomBinding{}
		if err := rows.Scan(&binding.APIKeyID, &binding.RoomID, &binding.GroupID, &binding.RateMultiplier); err != nil {
			return nil, err
		}
		bindings[binding.APIKeyID] = binding
	}
	return bindings, rows.Err()
}

func isMissingAPIKeyRoomBindingTable(err error) bool {
	if err == nil {
		return false
	}
	var pqErr *pq.Error
	...
	return strings.Contains(message, "redstone_api_key_room_bindings") &&
		(strings.Contains(message, "no such table") || strings.Contains(message, "does not exist"))
}

func apiKeyRoomBindingsQuery(apiKeyIDs []int64, dialectName string) (string, []any) {
	if dialectName == dialect.Postgres {
		return `
			SELECT api_key_id, room_id, group_id, rate_multiplier
			FROM redstone_api_key_room_bindings
			WHERE api_key_id = ANY($1::bigint[])`, []any{pq.Array(apiKeyIDs)}
	}
	...
	return `SELECT api_key_id, room_id, group_id, rate_multiplier FROM redstone_api_key_room_bindings WHERE api_key_id IN (` + strings.Join(placeholders, ", ") + `)`, args
}
