...
import (
	...
	dbpredicate "github.com/silent-QAQ/redstoneapi/ent/predicate"
	dbproxy "github.com/silent-QAQ/redstoneapi/ent/proxy"
...
// GetByCRSAccountIDAndOwner resolves an external CRS identity inside one
// account owner's namespace. CRS IDs are external values and are not globally
// unique once users can independently import the same upstream account.
func (r *accountRepository) GetByCRSAccountIDAndOwner(ctx context.Context, crsAccountID string, ownerUserID int64) (*service.Account, error) {
	if crsAccountID == "" || ownerUserID <= 0 {
		return nil, nil
	}

	m, err := r.client.Account.Query().
		Where(dbaccount.ParentAccountIDIsNil(), dbaccount.OwnerUserIDEQ(ownerUserID)).
		Where(func(s *entsql.Selector) {
			s.Where(sqljson.ValueEQ(dbaccount.FieldExtra, crsAccountID, sqljson.Path("crs_account_id")))
		}).
		Only(ctx)
