...
func NewAffiliateRepository(client *dbent.Client, db *sql.DB) service.AffiliateRepository {
	...
	repo.wallet, _ = wallet.NewService(walletRepository)
	return repo
}
...
FROM cleared`, userID)
		...
		if r.wallet == nil {
			return errors.New("redstone wallet is not configured for affiliate transfer")
		}
		walletKey := fmt.Sprintf("affiliate-transfer-%d-%d", userID, time.Now().UTC().UnixNano())
		walletResult, err := r.wallet.CreditInExecutor(txCtx, txClient, wallet.CreditRequest{
			UserID: userID, Asset: wallet.AssetNormal,
			Amount: decimal.NewFromFloat(transferred).Round(wallet.MonetaryScale),
			Reason: wallet.CreditSettlement,
			Reference: wallet.Reference{Type: "affiliate_transfer", ID: walletKey},
			IdempotencyKey: walletKey,
		})
		if err != nil {
			return fmt.Errorf("credit affiliate transfer wallet: %w", err)
		}
		if _, err := txClient.User.Update().Where(user.IDEQ(userID)).AddTotalRecharged(transferred).Save(txCtx); err != nil {
			return fmt.Errorf("record affiliate transfer recharge total: %w", err)
		}
		newBalance, _ = walletResult.BalanceAfter.Float64()
