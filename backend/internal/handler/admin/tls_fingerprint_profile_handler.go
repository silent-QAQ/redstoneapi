...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/model"
...
// ListOptions returns only fields required by account create/edit selectors.
func (h *TLSFingerprintProfileHandler) ListOptions(c *gin.Context) {
	profiles, err := h.service.List(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	...
	for _, profile := range profiles {
		if profile != nil {
			options = append(options, option{ID: profile.ID, Name: profile.Name})
		}
	}
