...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/handler/quotaview"
...
type UpdateBalanceRequest struct {
	...
	Asset     string  `json:"asset" binding:"omitempty,oneof=normal bound"`
	Notes     string  `json:"notes"`
}
...
func (h *UserHandler) UpdateBalance(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.users.balance.update", idempotencyPayload, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		var user *service.User
		var execErr error
		if walletAdmin, ok := h.adminService.(service.AdminWalletBalanceService); ok {
			asset := service.RedstoneWalletAsset(req.Asset)
			user, execErr = walletAdmin.UpdateUserWalletBalance(ctx, userID, req.Balance, req.Operation, asset, req.Notes, c.GetHeader("Idempotency-Key"))
		} else {
