...
// BindAPIKeySharingRoomRequest pins a key to one active account-sharing room.
// The server resolves the room's private group; callers cannot select a
// separate group in this endpoint.
type BindAPIKeySharingRoomRequest struct {
...
// BindSharingRoom binds the caller-owned API key to an accessible sharing
// room. The binding also sets the room's mapped group transactionally.
// PUT /api/v1/keys/:id/sharing-room
func (h *APIKeyHandler) BindSharingRoom(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	var req BindAPIKeySharingRoomRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	executeUserIdempotentJSON(c, "user.api_keys.bind_sharing_room", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		key, err := h.apiKeyService.BindSharingRoom(ctx, keyID, subject.UserID, req.RoomID)
		if err != nil {
			return nil, err
		}
		return dto.APIKeyFromService(key), nil
...
// UnbindSharingRoom removes the room binding from a caller-owned API key.
// DELETE /api/v1/keys/:id/sharing-room
func (h *APIKeyHandler) UnbindSharingRoom(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	key, err := h.apiKeyService.UnbindSharingRoom(c.Request.Context(), keyID, subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
