...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if apiKey.SharingRoomID != nil && *apiKey.SharingRoomID > 0 {
			ctx = context.WithValue(ctx, ctxkey.SharingRoomID, *apiKey.SharingRoomID)
		}
...
func validateAPIKeyGroupAllowed(apiKey *service.APIKey) bool {
	...
	// Room-mode keys are authorized by the Redstone room projection and lease
	// guard. Public rooms intentionally do not require a legacy
	// user_allowed_groups row; private-room ownership is checked there too.
	if apiKey.SharingRoomID != nil && *apiKey.SharingRoomID > 0 {
		return true
	}
	group := apiKey.Group
