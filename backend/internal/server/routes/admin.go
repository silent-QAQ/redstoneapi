...
// RedstoneAdminHandlers groups optional Redstone extensions registered under
// the existing protected /admin route group. Keeping this optional preserves
// focused route tests that construct only upstream handlers.
type RedstoneAdminHandlers struct {
...
func RegisterAdminRoutes(
	...
	redstoneHandlers ...RedstoneAdminHandlers,
) {
	var redstone RedstoneAdminHandlers
	if len(redstoneHandlers) > 0 {
		redstone = redstoneHandlers[0]
	}
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// 交易行资金结算。注册在既有 admin 组内，继承管理员认证、
		// 审计、合规确认及 step-up 保护，不创建平行控制面。
		market.RegisterAdminRoutes(admin, redstone.Market, stepUpAuth)
		sharing.RegisterAdminRoutes(admin, redstone.Sharing, stepUpAuth)
