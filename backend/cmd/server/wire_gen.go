...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	redstoneWalletPort, err := repository.NewRedstoneWalletPort(db)
	if err != nil {
		return nil, err
	}
	promoService := service.ProvidePromoService(promoCodeRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator, redstoneWalletPort)
	...
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, redstoneWalletPort)
	userService := service.ProvideUserService(userRepository, settingRepository, apiKeyAuthCacheInvalidator, billingCache, redstoneWalletPort)
	redeemCache := repository.NewRedeemCache(redisClient)
	redeemService := service.ProvideRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, affiliateService, redstoneWalletPort)
	...
	usageService := service.ProvideUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator, redstoneWalletPort)
	...
	antigravityGatewayService := service.ProvideAntigravityGatewayService(accountRepository, gatewayCache, schedulerSnapshotService, antigravityTokenProvider, rateLimitService, httpUpstream, settingService, internal500CounterCache, accessGuard)
	geminiMessagesCompatService := service.ProvideGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, antigravityGatewayService, configConfig, accessGuard)
	...
	manager := cluster.ProvideManager(configConfig, db, redisClient)
	backupService := service.ProvideBackupService(settingRepository, configConfig, secretEncryptor, backupObjectStoreFactory, dbDumper, manager)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, redstoneWalletPort)
	...
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db, manager)
	...
	marketRepository, err := market.NewPostgresRepository(db)
	if err != nil {
		return nil, err
	}
	deliveryContentResolver, err := repository.NewMarketDeliveryResolver(configConfig)
	if err != nil {
		return nil, err
	}
	deliveryScanner, err := market.ProvideDeliveryScanner(configConfig)
	if err != nil {
		return nil, err
	}
	marketService, err := market.ProvideService(marketRepository, paymentConfigService, deliveryContentResolver, deliveryScanner)
	if err != nil {
		return nil, err
	}
	marketHandler := market.NewHandler(marketService)
	postgresRepository, err := wallet.NewPostgresRepository(db)
	if err != nil {
		return nil, err
	}
	walletService, err := wallet.NewService(postgresRepository)
	if err != nil {
		return nil, err
	}
	operationsService, err := operations.NewService(db, walletService)
	if err != nil {
		return nil, err
	}
	...
	sharingService, err := sharing.NewService(sharingPostgresRepository, walletService)
	if err != nil {
		return nil, err
	}
	sharingHandler := sharing.NewHandler(sharingService)
	walletHandler := wallet.NewHandler(walletService)
	controlledaccountService, err := controlledaccount.ProvideService(db, configConfig)
	if err != nil {
		return nil, err
	}
	...
	deliveryScanWorker := market.ProvideDeliveryScanWorker(marketService, configConfig)
	leaseExpiryWorker := sharing.ProvideLeaseExpiryWorker(sharingPostgresRepository, configConfig)
	...
	opsCleanupService := service.ProvideOpsCleanupService(opsRepository, db, redisClient, configConfig, channelMonitorService, settingRepository, opsService, manager)
	...
	channelMonitorRunner := service.ProvideChannelMonitorRunner(channelMonitorService, settingService, manager)
	channelMonitorV2Aggregator := service.ProvideChannelMonitorV2Aggregator(channelMonitorV2Repository, db, settingService, manager)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, promptService, manager, settlementWorker, deliveryScanWorker, leaseExpiryWorker, controlledaccountHandler)
	application := &Application{
		Server:                    httpServer,
		PromptAudit:               promptService,
		MarketSettlement:          settlementWorker,
		MarketDeliveryScan:        deliveryScanWorker,
		SharingLeaseExpiryCleanup: leaseExpiryWorker,
		ControlledAccountHandler:  controlledaccountHandler,
		Cleanup:                   v,
	}
