...
import (
	...
	"github.com/silent-QAQ/redstoneapi/internal/repository"
	"github.com/silent-QAQ/redstoneapi/internal/securityaudit"
...
type Application struct {
	...
	MarketSettlement             *market.SettlementWorker
	MarketDeliveryScan           *market.DeliveryScanWorker
	SharingLeaseExpiryCleanup    *sharing.LeaseExpiryWorker
...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	wire.Build(
		...
		payment.ProviderSet,
		cluster.ProvideManager,
		wire.Bind(new(service.ClusterTaskLeaseCoordinator), new(*cluster.Manager)),
		market.NewPostgresRepository,
		repository.NewMarketDeliveryResolver,
		market.ProvideDeliveryScanner,
		wire.Bind(new(market.SellerRechargeMultiplierProvider), new(*service.PaymentConfigService)),
		market.ProvideService,
		...
		wire.Bind(new(service.SharingAccessGuard), new(*sharing.AccessGuard)),
		sharing.NewService,
		sharing.NewHandler,
		controlledaccount.ProvideService,
		controlledaccount.ProvideHandler,
		wallet.NewPostgresRepository,
		wire.Bind(new(wallet.Repository), new(*wallet.PostgresRepository)),
		wallet.NewService,
		wallet.NewHandler,
		operations.NewService,
		...
		// Application struct
		wire.Struct(new(Application), "Server", "PromptAudit", "MarketSettlement", "MarketDeliveryScan", "SharingLeaseExpiryCleanup", "ControlledAccountHandler", "Cleanup"),
	)
	return nil, nil
}
...
func provideCleanup(
	...
	return func() {
		...
		parallelSteps := []cleanupStep{
			{"RedstoneClusterManager", func() error {
			...
			{"MarketSettlementWorker", func() error {
				if marketSettlement != nil {
					marketSettlement.Stop()
				}
				return nil
			}},
			{"MarketDeliveryScanWorker", func() error {
				if marketDeliveryScan != nil {
					marketDeliveryScan.Stop()
				}
				return nil
			}},
			{"SharingLeaseExpiryCleanup", func() error {
				if sharingLeaseExpiryCleanup != nil {
					sharingLeaseExpiryCleanup.Stop()
				}
				return nil
			}},
		}
